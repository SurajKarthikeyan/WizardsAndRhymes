...
//////////////////////////////////////////////////////
	...
	float2 foveatedUV = UV_0;

	#if defined(SUPPORTS_FOVEATED_RENDERING_NON_UNIFORM_RASTER)
	if (_FOVEATED_RENDERING_NON_UNIFORM_RASTER)
	{
		foveatedUV = RemapFoveatedRenderingNonUniformToLinear(foveatedUV);
	}
	#endif

	half4 g = SampleTex2D(PASS_TEXTURE_2D(_BloomTex, sampler_linear_clamp_BloomTex), foveatedUV);
	//g = 0;
	...
	//return lerp(half4(0.1,0,0,0), half4(0,0.1,0,0), unity_StereoEyeIndex);
		...
		half4 glare = 0;
		#ifdef MK_GLARE_1
			glare = SampleTex2D(PASS_TEXTURE_2D(_Glare0Tex, sampler_linear_clamp_Glare0Tex), foveatedUV) * GLARE0_INTENSITY;
		#endif
		#ifdef MK_GLARE_2
			glare += SampleTex2D(PASS_TEXTURE_2D(_Glare1Tex, sampler_linear_clamp_Glare1Tex), foveatedUV) * GLARE1_INTENSITY;
		#endif
		#ifdef MK_GLARE_3
			glare += SampleTex2D(PASS_TEXTURE_2D(_Glare2Tex, sampler_linear_clamp_Glare2Tex), foveatedUV) * GLARE2_INTENSITY;
		#endif
		#ifdef MK_GLARE_4
			glare += SampleTex2D(PASS_TEXTURE_2D(_Glare3Tex, sampler_linear_clamp_Glare3Tex), foveatedUV) * GLARE3_INTENSITY;
		#endif
		#ifdef MK_NATURAL
	...
	#ifdef MK_LENS_FLARE
		g.rgb += SampleTex2DCircularChromaticAberration(PASS_TEXTURE_2D(_LensFlareTex, sampler_linear_clamp_LensFlareTex), foveatedUV, LENS_FLARE_CHROMATIC_ABERRATION).rgb;
	#endif
